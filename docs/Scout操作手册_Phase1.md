...
**当前阶段**：Phase 1 完成（2026-04-18）  
**下一步**：Phase 2A — 推荐生成 + 方向判断 Agent + V3 采集 + 财务一体化
...
| 无推荐 Agent | Phase 1 只采集+分析，不推股 | Phase 2A 启用（混合制：规则 + Sonnet）|
| 无财务一体化 | `stock_financials` 表预建但未填 | Phase 2A 接 AkShare 财务接口 |
| 无动机漂移检测 | `motivation_drift_log` 预建未用 | Phase 2B 启用 180 天周期检测 |
| 无事件串关联 | `event_chain_id` 预留字段未使用 | Phase 2C 启用 |
| DirectionJudge 简化版 | 只做周报总结 + 基础投票；无深度交叉验证 | Phase 2B 升级 Sonnet 版 |
| 推送只到 push_outbox | 真实推送到 OpenClaw / 邮件需要外部 worker | Phase 2A 加 consumer |

---

## 12 Phase 2A 准备

当 Phase 1 稳定运行 1-2 周后，可以进入 Phase 2A。

**Phase 2A 目标**：

1. **推荐 Agent**（两阶段：规则过滤 + Sonnet 深度评估）
2. **V3 Playwright**（韩国关税厅真采集）
3. **股票财务一体化**（AkShare 接口 + Piotroski F-Score 计算）
4. **related_stocks 填充**（每个 watchlist 行业 → 5-10 股）
5. **motivation_detail 完整评估**（v1.54 6 维度）
6. **推送 consumer**（从 push_outbox 真发到邮件/Telegram/OpenClaw）

**开发入口**（Phase 2A 新文件建议）：

- `agents/recommend_gate_agent.py` — 规则过滤层
- `agents/recommend_llm_agent.py` — Sonnet 深度评估层
- `infra/data_adapters/korea_customs_playwright.py` — V3 JS 渲染
- `agents/financial_agent.py` — 财务数据填充
- `infra/push_consumer.py` — push_outbox 真送达

...
- **2026-04-25 大盘点** (subagent 79 条, [docs/Scout_无效代码盘点_2026-04-25.md](Scout_无效代码盘点_2026-04-25.md)):
  - **A 类 50 条** — 清理待 final review (下次会话清醒头脑时批量处理)
  - **B 类 9 条 + C 类 20 条 = 29 条** — 进 4+3 台账 ⏳ 待 verify 队列 (蓝图 vs 实战逐条对照)
...
**状态变更**: 未启动 → **推迟到 Phase 4-5**（v1.69 unknowns 表上线后）

**关键发现**: 当前数据反映"d4 base<40 强制降 candidate"规则的蓝图前提存在偏差。

- B 级 24 条**全部 d4_base=20**，total 60.62-74.69
- candidate 150 条**全部 d4_base=20**
- A 级 8 条**全部 d4_base=100**
- 当前数据中 **d4_base=50 这一档 0 条**

**本质偏差**:

| 蓝图原意 | 当前实装语义 |
|---|---|
| 行业级判断: 产出兑现不足（v1.43 节能环保/数字创意那种） | Scout 自身能力评判: Scout 90 天内没采到信号 |

d4_base=20 在当前阶段大概率是 **Scout 信号采集覆盖不足**（V1/V3 月频 + paper_d4 TD-003 仍每天 27 errors），不是行业产出真的弱。

按蓝图字面实装会把 **"Scout 自身覆盖不足" 惩罚到推荐质量上** —— 这正是 [docs/Scout_设计意图与实现偏离.md](Scout_设计意图与实现偏离.md) 第八节"市场缺口识别"讲的同一类问题。

**不实装的理由**:

1. 区分"Scout 没采到" vs "行业真的弱" 需要 v1.69 unknowns 表（Phase 4-5 自省机制）
2. 如果在 Scout 不能区分前实装，会大幅压低 A/B 级输出（24+ 条 B 级降 candidate），与"找下一个 AI 链"目标背道而驰
3. TD-003 SemanticScholar 429 持续累积 = d4 信号采集本身不健康，TD-014a 实装等于把另一个 TD 的副作用变成推荐过滤

**蓝图规则前提待重审**:

蓝图 v1.52 阶段三"d4<6 force-downgrade"这条规则需要补充前提:

- **候选补充 1**: "industry 进入 watchlist 超过 N 天" 前提（给信号采集热身期）
- **候选补充 2**: 与"unknowns 表"联动（区分"行业真弱" vs "Scout 没采到"）
- **候选补充 3**: 阈值从 base<40 调到 base<20（只惩罚极端缺失，不惩罚一般缺失）

**下次决策点**: v1.69 unknowns 表实装时，配套重审这条规则的语义边界。

...
蓝图 v1.61 **没有明文规定**估值 veto，但实证发现：002371/688012 d6 score = 25（PEG ≈ 4-5，明显高估）仍获 level=A（total=90.94）。**估值过高没有 down-shift 推荐级别的机制**。
...
```python
mapping = {0: 0, 1: 0, 2: 50, 3: 70, 4: 85, 5: 100}
# 0/1: fatal, score 不参与判定
# 2: 刚过线
# 3: 有原型量产待验证

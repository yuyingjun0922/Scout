...
- **蓝图内容都要对比实际情况。蓝图 = 设计意图 + 参考数值, 不是最终权威**。
  ...
  下次会话开场后, 优先初始化 [docs/Scout_蓝图版本对齐.md](Scout_蓝图版本对齐.md) 为 "蓝图实战校验台账"
  (4+3 机制), 启动 47 项决策 verify pipeline。
...
**工作模式**: claude.ai (我) 不是 "修代码的助手", 是 **"蓝图与实战的对照判断者"**。每条规则实施前 verify 蓝图前提, 不成立时与用户讨论是改蓝图还是改代码。
